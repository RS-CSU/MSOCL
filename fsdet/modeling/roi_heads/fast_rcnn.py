...
class FastRCNNContrastiveOutputs(FastRCNNOutputs):
    """
    ...
    def __init__(
        ...
        self.crop_feat_con = crop_feat_con
    ...
    def contrastive_loss(self):
        contrastive_loss = self.criterion(self.box_cls_feat_con, self.crop_feat_con, self.batch_size, self.ious)
        # contrastive_loss = self.criterion(self.box_cls_feat_con, self.crop_feat_con, self.batch_size, self.crop_cls, self.gt_classes)
        # contrastive_loss = self.criterion(self.box_cls_feat_con, self.gt_classes, self.ious)
        return contrastive_loss

    def losses(self):
        if self.cl_head_only:
            return {'loss_contrast': self.contrastive_loss()}
        else:
            return {
                'loss_cls': self.softmax_cross_entropy_loss(),
                'loss_box_reg': self.box_reg_weight * self.smooth_l1_loss(),
                'loss_contrast': self.contrast_loss_weight * self.contrastive_loss(),
            }
