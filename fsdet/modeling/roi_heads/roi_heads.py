...
from detectron2.layers import ShapeSpec, ROIAlign
...
from .fast_rcnn import (
    FastRCNNOutputs,
    FastRCNNContrastiveOutputs,
    ROI_HEADS_OUTPUT_REGISTRY,
)
from ..utils import concat_all_gathered, select_all_gather
from ..contrastive_loss import ContrastLoss, ContrastiveHead
...
class StandardROIHeads(ROIHeads):
    ...
    def _init_box_head(self, cfg):
        ...
        self.roi_aligin = self.level_poolers = nn.ModuleList(
                ROIAlign(
                    (7, 7), spatial_scale=scale, sampling_ratio=sampling_ratio, aligned=True
                )
                for scale in pooler_scales
            )
...
class ContrastiveROIHeads(StandardROIHeads):
    ...
    def forward(self, images, features, features_crop, proposals, targets=None):
        """
        See :class:`ROIHeads.forward`.
            proposals (List[Instance]): fields=[proposal_boxes, objectness_logits]
                post_nms_top_k proposals for each image， len = N
        ...
        if self.training:
            ...
            # has field [proposal_boxes, objectness_logits ,gt_classes, gt_boxes]
            proposals = self.label_and_sample_proposals(proposals, targets)
            crop_cls = cat([gt.gt_classes for gt in targets], dim=0)
        del targets
        ...
        if self.training:
            # FastRCNNOutputs.losses()
            # {'loss_cls':, 'loss_box_reg':}
            features_crop_list = [features_crop[f] for f in self.in_features]
            losses = self._forward_box(features_list, proposals, crop_cls, features_crop_list)  # get losses from fast_rcnn.py::FastRCNNOutputs
            return proposals, losses  # return to rcnn.py line 201
        else:
            pred_instances = self._forward_box(features_list, proposals, crop_cls, features_crop)
            return pred_instances, {}

    def _forward_box(self, features, proposals, crop_cls, features_crop):
        # P = [x.proposal_boxes for x in proposals]
        ...
        if self.training:
            # print(self.roi_aligin[0])
            # a = features_crop[0][0]
            # roi = torch.from_numpy(
            #     np.array([[0, 0, 0, features_crop[0][0].shape[1], features_crop[0][0].shape[2]],
            #               [1, 0, 0, features_crop[0].shape[3], features_crop[0].shape[3]]])).cuda().float()

            # 使用roi aligin的结果
            # crop_feat_contrast = []
            # for f in range(len(self.in_features)):
            #     roi = torch.from_numpy(
            #     np.array([[i, 0, 0, features_crop[f][i].shape[1], features_crop[f][i].shape[2]] for i in range(self.batch_size)])).cuda().float()
            #     crop_feat = self.roi_aligin[f](features_crop[f], roi)
            #     crop_feat_box = self.box_head(crop_feat)
            #     crop_feat_con = self.encoder(crop_feat_box)
            #     crop_feat_contrast.append(crop_feat_con)
            # del crop_feat
            ...
            for f in range(len(self.in_features)):
                # print(f)
                # print(features_crop[f].shape)
                if features_crop[f].shape != (6, 256, 7, 7):
                    crop_feat = torch.nn.Upsample(size=(7, 7), mode='bilinear', align_corners=False)(features_crop[f])
                else:
                    crop_feat = features_crop[f]
                crop_feat_box = self.box_head(crop_feat)
                crop_feat_con = self.encoder(crop_feat_box)
                crop_feat_contrast.append(crop_feat_con)
            del crop_feat
            crop_feat_contrast = crop_feat_contrast[:4]

            # 使用特征融合
            # crop_feat = []
            # for f in range(len(self.in_features)):
            #     if features_crop[f].shape != (6, 256, 7, 7):
            #         crop_feat_ = torch.nn.Upsample(size=(7, 7), mode='bilinear', align_corners=True)(features_crop[f])
            #     else:
            #         crop_feat_ = features_crop[f]
            #     crop_feat.append(crop_feat_)
            # for i in range(len(self.in_features) - 1):
            #     crop_fusion = crop_feat[3] + crop_feat[i]
            #
            # crop_feat_box = self.box_head(crop_fusion)
            # crop_feat_contrast = self.encoder(crop_feat_box)
            # # crop_feat_contrast.append(crop_feat_con)
            # # del crop_feat

            # pass
            # crop_feat = self.box_head(features_crop)
            # crop_feat_contrast = self.encoder(crop_feat)
            # del crop_feat
        else:
            crop_feat_contrast = None


        outputs = FastRCNNContrastiveOutputs(
            self.box2box_transform,
            pred_class_logits,
            pred_proposal_deltas,
            proposals,
