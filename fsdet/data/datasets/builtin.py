...
def register_all_dior(root="datasets"):
    SPLITS = [
        ("dior_trainval", "VOC2007", "trainval"),
        ("dior_train", "VOC2007", "train"),
        ("dior_val", "VOC2007", "val"),
        ("dior_test", "VOC2007", "test"),
    ]
    for name, dirname, split in SPLITS:
        year = 2007 if "2007" in name else "dior"
        register_dior(name, os.path.join(root, dirname), split, year)
        MetadataCatalog.get(name).evaluator_type = "pascal_voc"
    ...
    # print(METASPLITS)
    # ('voc_2007_trainval_all1_1shot', 'DIOR', 'all_1shot_split_1_trainval', 'base_novel_1', 1)
    # ('voc_2007_trainval_novel3_10shot', 'DIOR', 'novel_10shot_split_3_trainval', 'novel3', 3)
    for name, dirname, split, keepclasses, sid in METASPLITS:
        # year = 2007 if "2007" in name else dior
        register_meta_dior(name,
                                 _get_builtin_metadata("dior_fewshot"),
                                 os.path.join(root, dirname), split, "dior",
                                 keepclasses, sid)
        MetadataCatalog.get(name).evaluator_type = "pascal_voc"
        ...
        for sid in range(1, 5):
            for shot in [1, 2, 3, 5, 10]:
                for year in ["dior"]:
                    for seed in range(100):
                        seed = '' if seed == 0 else '_seed{}'.format(seed)
                        name = "{}_trainval_{}{}_{}shot{}_{}ploidy".format(
                            year, prefix, sid, shot, seed, ploidy)
                        dirname = "{}".format(year)
                        img_file = "{}_{}shot_split_{}_trainval".format(
                            prefix, shot, sid)
