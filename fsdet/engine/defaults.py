...
def default_argument_parser():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/DIOR/split1/10shot_OCL.yml",
        # default="../models/FSCE/final_split3/seed2/10ceshi3/config.yaml",
        metavar="FILE",
    ...
    parser.add_argument("--eval-iter", type=int, default=3000,
...
class DefaultTrainer(SimpleTrainer):
    ...
    def __init__(self, cfg):
        ...
        # Whether to crop the picture as a network branch（是否对图片进行裁剪作为网络分支）
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            # print(data_loader)
            # target_layer = model.roi_heads.box_predictor.cls_score
            # cam = GradCAM(model=model, target_layers=[model.roi_heads.box_predictor.cls_score], use_cuda=False)
            # target_category = None
            # grayscale_cam = cam(input_tensor=images.tensor[0], target_category=target_category)  # [batch, 224,224]
            # grayscale_cam = grayscale_cam[0]
            # visualization = show_cam_on_image(batched_inputs[0], grayscale_cam)  # (224, 224, 3)
            # visualization.show()
            #counts = Counter()
            # total_flops = []
            # for idx, data in enumerate(data_loader):  # noqa
            ...
            #     print(macs, params)
            #     # df = summary(model,inputs)
            #     # print(df)
            #     pass
            #     # flops = FlopCountAnalysis(model, inputs)
            #     # print("flops:", flops)
            #     # i = inputs[0]["image"]
            #     # stat(model, inputs[0]["image"].shape)
            #     pass
